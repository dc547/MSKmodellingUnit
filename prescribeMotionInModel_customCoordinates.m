...
function prescribeMotionInModel_customCoordinates(Model_In, Mot_In, Model_Out,not_prescr_coords)
...
%         - not_prescr_coords - array with coordinates names not to be
%         prescribed (they need to be in order of appearence in the model)
...
for i=0:nCoords-1
    ...
    if nt_coord > length(not_prescr_coords)
    nt_coord =1;
    end
        if strcmp(currentcoord.getName(),not_prescr_coords(nt_coord))
            disp('Locked Coord');
            %%
        else
